...
const {
  ...
  refresh,
  error: loadError,
} = await useFetch<AdminEvent[]>("/api/admin/events", {
  default: () => [],
});
...
async function saveEvent() {
  ...
  try {
    ...
    formError.value =
      error?.statusMessage || error?.message || "Could not save event.";
  } finally {
    saving.value = false;
  }
}
...
useSiteSeo({
  title: "Manage events",
  description: "Staff event management.",
  noindex: true,
});
</script>
  ...
  <div class="admin-page">
    <div class="admin-page-head">
      <div>
        ...
        <p class="admin-page-lead">
          Add special one-off events to the public calendar. Bible Study and
          Sunday Service are generated automatically each week.
        </p>
      </div>
      <div class="admin-page-actions">
        <button class="btn btn-ghost-dark" type="button" @click="startCreate">
          New event
        </button>
      </div>
    </div>
    ...
    <div class="admin-grid">
      ...
      <section class="admin-panel list-panel">
        ...
        <p v-if="loadError" class="admin-error">
          Could not load events. Check Supabase configuration.
        </p>
        <p v-else-if="pending" class="admin-empty">Loading…</p>
        <p v-else-if="!events?.length" class="admin-empty">
          ...
          <li v-for="event in events" :key="event.id" class="admin-list-item">
            <div>
              <div class="admin-list-title-row">
                ...
                <span
                  class="admin-pill"
                  :class="event.published ? '' : 'muted'"
                >
                  {{ event.published ? "Published" : "Draft" }}
                </span>
              </div>
              <p class="admin-meta">{{ formatWhen(event.starts_at) }}</p>
              <p v-if="event.location" class="admin-meta">
                {{ event.location }}
              </p>
              <p v-if="event.summary" class="admin-message">
                {{ event.summary }}
              </p>
              <NuxtLink
                class="admin-link"
