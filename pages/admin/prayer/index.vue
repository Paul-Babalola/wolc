...
const {
  ...
  error,
  refresh,
} = await useFetch<PrayerRequest[]>("/api/admin/prayer-requests", {
  default: () => [],
});
...
async function toggleHandled(item: PrayerRequest) {
  ...
  const label = item.name || "Anonymous";
  const ok = await confirmAction({
    title: isHandled ? "Reopen request" : "Mark as handled",
    message: isHandled
      ? `Reopen the prayer request from ${label}? It will show as open again.`
      : `Mark the prayer request from ${label} as handled?`,
  ...
  try {
    ...
    actionError.value =
      err?.statusMessage || "Could not update prayer request.";
  } finally {
...
useSiteSeo({
  ...
  description: "Staff prayer request management.",
  noindex: true,
});
</script>
...
<template>
  <div class="admin-page">
    <div class="admin-page-head">
      <div>
        ...
        <p class="admin-page-lead">
          Review prayer requests submitted on the public site. Private requests
          are only visible here and in staff email notifications.
        </p>
      </div>
    ...
    <section class="admin-panel">
      ...
      <p v-else-if="!filtered.length" class="admin-empty">
        ...
        <li v-for="item in filtered" :key="item.id" class="admin-list-item">
          <div>
            <div class="admin-list-title-row">
              ...
              <span v-if="item.is_private" class="admin-pill info"
                >Private</span
              >
            </div>
